...
-- 8. Criar função de limpeza automática de tarefas antigas
...
-- 9. Inserir tarefas exemplo para usuários existentes (opcional - remover se não quiser dados de exemplo)
/*
...
-- - Limpeza automática de tarefas antigas (24h após conclusão)

-- 🔧 CONFIGURAÇÃO ADICIONAL NECESSÁRIA:
-- Para ativar a limpeza automática, execute os comandos abaixo no Supabase:
-- 1. Habilitar extensão pg_cron (apenas administradores):
--    SELECT cron.schedule('cleanup-completed-tasks', '0 2 * * *', 'SELECT cleanup_completed_tasks();');
-- 
-- 2. Ou execute manualmente quando necessário:
--    SELECT cleanup_completed_tasks();
--
-- ⚠️ POLÍTICA DE LIMPEZA:
-- Tarefas marcadas como "Concluída" serão automaticamente removidas após 24 horas
-- Esta política visa manter o banco de dados limpo e otimizado